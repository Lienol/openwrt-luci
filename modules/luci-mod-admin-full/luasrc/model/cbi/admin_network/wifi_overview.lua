	...
	<fieldset class="cbi-section-node">
		<table class="table">
			...
			<% if #wnets > 0 then %>
				...
				<tr class="tr cbi-rowstyle-<%=1 + ((i-1) % 2)%>">
					<td class="td col-2 center middle" id="<%=net:id()%>-iw-signal">
						<span class="ifacebadge" title="<%:Not associated%>"><img src="<%=resource%>/icons/signal-<%= disabled and "none" or "0" %>.png" /> 0%</span>
					</td>
					<td class="td col-7 left middle" id="<%=net:id()%>-iw-status" data-network="<%=net:id()%>" data-disabled="<%= disabled and "true" or "false" %>">
